CREATE OR REPLACE TABLE `bigquery-tutorial-480009.A2.Fact_HbA1c`
(
  -- Foreign Keys
  ...
  -- HbA1c Facts (Measures)
  HbA1c FLOAT64 NOT NULL,
  Estimated_Avg_Glucose FLOAT64 NOT NULL,
  HbF FLOAT64 NOT NULL
)
AS
SELECT
  ...
  -- HbA1c FACTS (Measures)
  COALESCE(f.`HbA1c %`, 0.0) AS HbA1c,
  COALESCE(f.`Estimated Avg Glucose _mg_dL_`, 0.0) AS Estimated_Avg_Glucose,
  COALESCE(f.`HbF %`, 0.0) AS HbF

CREATE OR REPLACE TABLE `bigquery-tutorial-480009.A2.Fact_Thyroid_Profile`
(
  -- Foreign Keys
  ...
  -- Thyroid Profile Facts (Measures)
  TT3 INT64 NOT NULL,
  TT4 FLOAT64 NOT NULL,
  TSH FLOAT64 NOT NULL
)
AS
SELECT
  ...
  -- THYROID PROFILE FACTS (Measures)
  COALESCE(f.`TT3 _ng_dL_`, 0) AS TT3,
  COALESCE(f.`TT4 _µg_dL_`, 0.0) AS TT4,
  COALESCE(f.`TSH _µIU_mL_`, 0.0) AS TSH

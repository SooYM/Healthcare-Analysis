CREATE OR REPLACE TABLE `bigquery-tutorial-480009.A2.Fact_Glucose_Diagnopath`
(
  -- Foreign Keys
  ...
  -- Glucose Diagnopath Facts (Measures)
  FBS INT64 NOT NULL,
  PLBS INT64 NOT NULL
)
AS
SELECT
  ...
  -- GLUCOSE DIAGNOPATH FACTS (Measures)
  COALESCE(f.`FBS _mg_dL_`, 0) AS FBS,
  COALESCE(f.`PLBS _mg_dL_`, 0) AS PLBS

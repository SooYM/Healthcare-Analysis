CREATE OR REPLACE TABLE `bigquery-tutorial-480009.A2.Fact_Calcium_Phos`
(
  -- Foreign Keys
  ...
  -- Calcium & Phosphorus Facts (Measures)
  Calcium FLOAT64 NOT NULL,
  Phosphorus FLOAT64 NOT NULL
)
AS
SELECT
  ...
  -- CALCIUM & PHOS FACTS (Measures)
  COALESCE(f.`Calcium _mg_dL_`, 0.0) AS Calcium,
  COALESCE(f.`Phosphorus _mg_dL_`, 0.0) AS Phosphorus
